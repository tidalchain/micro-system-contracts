...
import type { Wallet } from "@zkamoeba/micro-web3";
import { serialize } from "@zkamoeba/micro-web3/build/src/utils";
...
describe("BootloaderUtilities tests", function () {
  ...
  describe("EIP-712 transaction", function () {
    it("check hashes", async () => {
      const eip712Tx = await wallet.populateTransaction({
        ...
        customData: {
          gasPerPubdata: micro.utils.DEFAULT_GAS_PER_PUBDATA_LIMIT,
        },
      });
      const signedEip712Tx = await wallet.signTransaction(eip712Tx);
      const parsedEIP712tx = micro.utils.parseTransaction(signedEip712Tx);

      const eip712TxData = signedTxToTransactionData(parsedEIP712tx)!;
      const expectedEIP712TxHash = parsedEIP712tx.hash;
      const expectedEIP712SignedHash = micro.EIP712Signer.getSignedDigest(eip712Tx);
  ...
  describe("legacy transaction", function () {
    it("check hashes", async () => {
      ...
      const parsedTx = micro.utils.parseTransaction(txBytes);
  ...
  describe("EIP-1559 transaction", function () {
    it("check hashes", async () => {
      ...
      const parsedEIP1559tx = micro.utils.parseTransaction(signedEip1559Tx);
  ...
  describe("EIP-1559 transaction", function () {
    it("check hashes", async () => {
      ...
      const parsedEIP2930tx = micro.utils.parseTransaction(signedEip2930Tx);
