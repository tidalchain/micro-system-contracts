...
import type { MicroArtifact } from "@zkamoeba/hardhat-micro-deploy/dist/types";
import type { BytesLike } from "ethers";
import * as hre from "hardhat";
import { ethers, network } from "hardhat";
import * as micro from "@zkamoeba/micro-web3";
import type { Contract } from "@zkamoeba/micro-web3";
...
export async function loadArtifact(name: string): Promise<MicroArtifact> {
...
export async function setCode(address: string, bytecode: BytesLike) {
  ...
  const deployment = {
    bytecodeHash: micro.utils.hashBytecode(bytecode),
    newAddress: address,
