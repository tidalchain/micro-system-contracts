...
import { utils } from "@zkamoeba/micro-web3";
...
const params = {
  ...
  TRANSFER_FEE_TO_SERVER_SELECTOR: getPaddedSelector("FeePool", "addFee"),
  BATCH_EVENT_CREATE_BATCH_SELECTOR: getPaddedSelector("BatchEvent", "onCreateBatch"),
...
async function main() {
  ...
  const provedBatchBootloader = preprocess.preprocess(bootloader, {
    BOOTLOADER_TYPE: "proved_batch",
  });
  console.log("Preprocessing playground block bootloader");
  const playgroundBatchBootloader = preprocess.preprocess(bootloader, {
    BOOTLOADER_TYPE: "playground_batch",
  });
  console.log("Preprocessing gas test bootloader");
  const gasTestBootloader = preprocess.preprocess(gasTestBootloaderTemplate, {
    BOOTLOADER_TYPE: "proved_batch",
  });
  ...
  const provedBootloaderWithTests = preprocess.preprocess(bootloaderWithTests, {
    BOOTLOADER_TYPE: "proved_batch",
  });
