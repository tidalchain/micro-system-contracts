...
import { Provider, Wallet } from "@zkamoeba/micro-web3";
import { hashBytecode } from "@zkamoeba/micro-web3/build/src/utils";
...
const MAX_COMBINED_LENGTH = 60000;
...
class MicroDeployer {
  ...
  // Returns the current default account bytecode on micro
  async currentDefaultAccountBytecode(): Promise<string> {
    const micro = await this.deployer.zkWallet.getMainContract();
    return await micro.getL2DefaultAccountBytecodeHash();
  }
  ...
  async checkShouldUpgradeDefaultAA(defaultAccountBytecode: string) {
    ...
    // If the bytecode is not the same as the one deployed on micro, we need to add it to the deployment
  ...
  async currentBootloaderBytecode(): Promise<string> {
    const micro = await this.deployer.zkWallet.getMainContract();
    return await micro.getL2BootloaderBytecodeHash();
  }
...
async function main() {
  ...
  program
    ...
    .action(async (cmd) => {
      ...
      const microDeployer = new MicroDeployer(deployer, gasPrice, nonce);
      if (cmd.bootloader) {
        await microDeployer.processBootloader();
      }

      if (cmd.defaultAa) {
        await microDeployer.processDefaultAA();
      }

      if (cmd.systemContracts) {
        const dependenciesToPublish = await microDeployer.prepareContractsForPublishing();
        await microDeployer.publishDependencies(dependenciesToPublish);
      }

      const result = microDeployer.returnResult();
