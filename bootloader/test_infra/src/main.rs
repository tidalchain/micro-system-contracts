...
use micro_types::system_contracts::get_system_smart_contracts_from_dir;
use micro_types::{block::legacy_miniblock_hash, Address, L1BatchNumber, MiniblockNumber, U256};
use micro_types::{L2ChainId, Transaction};
...
fn execute_internal_bootloader_test() {
    ...
    for test_id in 1..=test_count {
        ...
        l1_batch_env.fee_account = micro_types::H160::from(u256_to_h256(U256::from(test_id)));
        let mut vm = Vm::new(
            l1_batch_env.clone(),
            system_env.clone(),
            storage.clone(),
