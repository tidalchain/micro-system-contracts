object "Bootloader" {
    ...
    object "Bootloader_deployed" {
        code {
            ...
            function FEE_POOL_ADDRESS() -> ret {
                ret := 0x0000000000000000000000000000000000008101
            }

            function BATCH_EVENT_ADDRESS() -> ret {
                ret := 0x0000000000000000000000000000000000008102
            }

            function DAO_ADDR() -> ret {
                ret := 0x0000000000000000000000000000000000008103
            }
            ...
            function processTx(
                ...
                switch getTxType(innerTxDataOffset) 
                    ...
                    default {
                        ...
                        tips := processL2Tx(txDataOffset, resultPtr, transactionIndex, gasPerPubdata)
                        <!-- @endif -->
            ...
            function getExecuteL1TxAndGetRefund(txDataOffset, gasForExecution) -> potentialRefund, success {
                ...
                success := MICRO_NEAR_CALL_executeL1Tx(
                    callAbi,
                    txDataOffset
                )
            ...
            function getGasPrice(
                ...
                if gt(baseFee, maxFeePerGas) {
                 ...
                 <!-- @if BOOTLOADER_TYPE=='proved_batch' -->
                if eq(txType, 2) {
                    gasPrice := add(baseFee, maxPriorityFeePerGas)
                    if gt(gasPrice, maxFeePerGas) {
                        gasPrice := maxFeePerGas
                        priorityFeePerGas := sub(maxFeePerGas, baseFee)
                    }
                }
                <!-- @endif -->
            }
            ...
            function processL2Tx(
                ...
                gasPrice, priorityFeePerGas := getGasPrice(txType, getMaxFeePerGas(innerTxDataOffset), getMaxPriorityFeePerGas(innerTxDataOffset))
                ...
                <!-- @if BOOTLOADER_TYPE=='proved_batch' -->
                tips := mul(priorityFeePerGas, sub(totalGasLimit, refund))
                <!-- @endif -->
                <!-- @if BOOTLOADER_TYPE=='playground_batch' -->
                tips := 0
                <!-- @endif -->
            ...
            function l2TxValidation(
                ...
                // Note, that it is assumed that `MICRO_NEAR_CALL_validateTx` will always return true
                ...
                if gasLimitForTx {
                    ...
                    isValid := MICRO_NEAR_CALL_validateTx(validateABI, txDataOffset, gasPrice)                    
                }
            ...
            function l2TxExecution(
                ...
                if gasLeft {
                    ...
                    newCompressedFactoryDepsPointer := MICRO_NEAR_CALL_markFactoryDepsL2(markingDependenciesABI, txDataOffset)
                ...
                default {
                    ...
                    success := MICRO_NEAR_CALL_executeL2Tx(
                        executeABI,
                        txDataOffset
                    )
            ...
            function MICRO_NEAR_CALL_validateTx(
                abi,
                txDataOffset,
                gasPrice
            ) -> ret {
            ...
            /// @param abi The nearCall ABI. It is implicitly used as gasLimit for the call of this function.
            /// @param txDataOffset The offset to the ABI-encoded Transaction struct.
            function MICRO_NEAR_CALL_executeL2Tx(
                abi,
                txDataOffset
            ) -> success {
            ...
            function MICRO_NEAR_CALL_markFactoryDepsL2(
                abi,
                txDataOffset
            ) -> newDataInfoPtr {
                let innerTxDataOffset := add(txDataOffset, 32)
            ...
            function refundCurrentL2Transaction(
                ...
                default {
                    ...
                    if gt(gasLeft, 0) {
                        ...
                        pop(MICRO_NEAR_CALL_callPostOp(
            ...
            function transferFeeToServer(amount, recipient) {
                debugLog("transferFeeToServer", amount)
                mstore(0, {{TRANSFER_FEE_TO_SERVER_SELECTOR}})
                mstore(4, recipient)
                ...
                if iszero(callSuccess) {
                    assertionError("Failed to record fee")
                }
            }

            function batchEventCreateBatch(batchNumber, operator) {
                debugLog("batchEventCreateBatch batch number", batchNumber)
                debugLog("batchEventCreateBatch operator", operator)
                mstore(0, {{BATCH_EVENT_CREATE_BATCH_SELECTOR}})
                mstore(4, batchNumber)
                mstore(36, operator)
                ...
                if iszero(callSuccess) {
                    assertionError("Failed to create batch")
                }
            }
            ...
            /// @param abi The nearCall ABI. It is implicitly used as gasLimit for the call of this function.
            /// @param txDataOffset The offset to the ABI-encoded Transaction struct.
            function MICRO_NEAR_CALL_executeL1Tx(
                abi,
                txDataOffset
            ) -> success {
                ...
                // We assume that addresses of smart contracts on micro and Ethereum
            ...
            <!-- @if BOOTLOADER_TYPE=='playground_batch' -->
            function MICRO_NEAR_CALL_ethCall(
                abi,
                txDataOffset,
                resultPtr
            ) {
            ...
            function MICRO_CATCH_NEAR_CALL() {
                debugLog("MICRO_CATCH_NEAR_CALL",0)
            ...
            function MICRO_NEAR_CALL_callPostOp(abi, paymaster, txDataOffset, txResult, maxRefundedGas) -> success {
            ...
            function validateTypedTxStructure(innerTxDataOffset) {
                ...
                switch txType
                    ...
                    case 255 {
                        ...
                        assertEq(or(eq(getTo(innerTxDataOffset), BATCH_EVENT_ADDRESS()), gt(getFrom(innerTxDataOffset), MAX_SYSTEM_CONTRACT_ADDR())), 1, "from in kernel space")
            ...
            /// `MICRO_CATCH_NEAR_CALL` function.
            ...
            for { 
                ...
                {
                    debugLog("ethCall", 0)
                    let tips :=  processTx(txDataOffset, resultPtr, transactionIndex, 0, GAS_PRICE_PER_PUBDATA)
                ...
                <!-- @if BOOTLOADER_TYPE=='playground_batch' -->
                {
                    ...
                    let tips :=  processTx(txDataOffset, resultPtr, transactionIndex, isETHCall, GAS_PRICE_PER_PUBDATA)
                    totalTips := add(totalTips, tips)
                }
                <!-- @endif -->
            ...
            // When the first batch starts, the system contract has not yet been deployed
            if gt(BATCH_NUMBER, 1) {
                if gt(totalTips, 0) {
                    // Transfering tips received in the block to the fee pool
                    transferFeeToServer(
                        totalTips,
                        OPERATOR_ADDRESS
                    )
                }
                batchEventCreateBatch(BATCH_NUMBER, OPERATOR_ADDRESS)
            }

            // Transfering all the ETH received in the block to the dao
            directETHTransfer(
                selfbalance(),
                DAO_ADDR() // official dao address
            )
