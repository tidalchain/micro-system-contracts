...
/// @dev The type id of micro's EIP-712-signed transaction.
...
/// @notice Structure used to represent micro transaction.
struct Transaction {
...
library TransactionHelper {
    ...
    /// @notice Encode hash of the micro native transaction type.
    ...
    function _encodeHashEIP712Transaction(Transaction calldata _transaction) private view returns (bytes32) {
        ...
        bytes32 domainSeparator = keccak256(
            abi.encode(EIP712_DOMAIN_TYPEHASH, keccak256("micro"), keccak256("2"), block.chainid)
        );
    ...
    function _encodeHashEIP2930Transaction(Transaction calldata _transaction) private view returns (bytes32) {
        ...
        //
        // Note, that on micro access lists are not supported and should always be empty.
        ...
        // On micro, access lists are always zero length (at least for now).
